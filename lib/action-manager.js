...
exports.ActionManager.prototype = {
        ...
        if (data.type == "login") {
            ...
            switch (data.data.name) {
                case "createGame":
                    ...
                    break;
                case "joinGame":
                    this.engine.joinGame(this.engine.getPlayer(client.id), data.data.data.game_id);
                    break;
            }
        }
    ...
    getGamesList: function(gamesList) {
        ...
        console.log(util.inspect(gamesList));
        return this.getData( "update", "games", gamesList );
    },

    getNewGameData: function(game) {
        return this.getData('new', 'Game', game);
    },

    getUpdateGameData: function(game) {
        return this.getData('update', 'Game', game);
    },
